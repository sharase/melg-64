/* ***************************************************************************** */
/* A C-program for MEMT44497-64-JUMP                                             */
...
static unsigned long long case_1(void) {
    ...
    lung = (x >> 1) ^ mag01[(int)(x & 1ULL)] ^ mt[mti+MM] ^ MAT3NEG(37, lung);
    mt[mti] = x ^ MAT3POS(14, lung);
...
static unsigned long long case_2(void) {
    x = (mt[mti] & MASKU) | (mt[mti+1] & MASKL);
    lung = (x >> 1) ^ mag01[(int)(x & 1ULL)] ^ mt[mti+(MM-NN)] ^ MAT3NEG(37, lung);
    mt[mti] = x ^ MAT3POS(14, lung);
    x = mt[mti] ^ (mt[mti] << SHIFT1);
    x = x ^ (mt[mti + LAG1] & MASK1);
    ++mti;
...
static unsigned long long case_4(void) {
    ...
    lung = (x >> 1) ^ mag01[(int)(x & 1ULL)] ^ mt[MM-1] ^ MAT3NEG(37, lung);
    mt[NN-1] = x ^ MAT3POS(14, lung);
    x = mt[mti] ^ (mt[mti] << SHIFT1);
    x = x ^ (mt[mti - LAG1over] & MASK1);
    mti = 0;
    genrand64_int64 = case_1;
